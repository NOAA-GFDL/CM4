...
# Sample run script to run the cm4p0 experiment
...
executable=/path/to/executable/fms_CM4p0.x
...
# Run the model
${mpiexec_prog} ${mpiexec_nopt} ${atm_ranks} ${mpiexec_topt} ${omp_threads} ${executable} : ${mpiexec_prog} ${mpiexec_nopt} ${ocean_ranks} ${executable} 2>&1 | tee ${workDir}/fms.out
if [ $? -ne 0 ]
then
