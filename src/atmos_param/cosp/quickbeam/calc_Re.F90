...
#ifdef COSP_GFDL
        call error_mesg ('calc_Re', &     
         'Error: Must specify a value for Np in each volume &
                       &with Morrison/Martin Scheme.', FATAL)
#else
...
#ifdef COSP_GFDL
        call error_mesg ('calc_Re', &     
       'Error: Must specify a value for vu for Modified Gamma &
                                        &distribution', FATAL)
#else
...
#ifdef COSP_GFDL
        call error_mesg ('calc_Re', &     
             'Error: Must specify Np or default value &    
                 &(p1=Dm [um] or p2=Np [1/kg]) for &    
                 &Modified Gamma distribution', FATAL)
#else
...
! :: ld = slope parameter (um)
  ...
  case(2)
    ...
    elseif (abs(p2+1) > 1E-8) then  ! lambda=ld has been specified as default
        ...
        call error_mesg ('calc_Re', &     
     'Error: Must specify Np or default value &    
         &(p1=No or p2=lambda) for Exponential distribution', FATAL)
#else
    print *, 'Error: Must specify Np or default value ', &
...
#ifdef COSP_GFDL
        call error_mesg ('calc_Re', &     
         'Variable Np not supported for Power Law distribution', FATAL)
#else
...
#ifdef COSP_GFDL
        call error_mesg ('calc_Re', &     
         'Variable Np not supported for Monodispersed distribution', FATAL)
#else
        print *, 'Variable Np not supported for ', &
...
! :: log_sigma_g = ln(geometric standard deviation)
  ...
  case(5)
        ...
        call error_mesg ('calc_Re', &     
             ...
             & when using a Log-Normal distribution', FATAL)
#else
    ...
    if( Np.eq.0 .and. (abs(p2+1) > 1E-8) ) then ! use default value of rg
        ...
        call error_mesg ('calc_Re', &     
             'Error: Must specify Np or default value&
                 &(p2=Rg or p1=Np) for Log-Normal distribution', FATAL)
#else
                print *, 'Error: Must specify Np or default value ', &
