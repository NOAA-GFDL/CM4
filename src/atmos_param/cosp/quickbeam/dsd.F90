...
#ifdef COSP_GFDL
        call error_mesg ('dsd.F90',  & 
         'Error: Must specify a value for Np in each volume with  &
                                         &Morrison/Martin Scheme.', FATAL)
#else
...
#ifdef COSP_GFDL
        call error_mesg ('dsd.F90',  & 
        'Error:  No default value for Dm or Np provided!  ', FATAL)
#else
...
#ifdef COSP_GFDL
        call error_mesg ('dsd.F90',  & 
         'Variable Re not supported for Power-Law distribution', FATAL)
#else
...
#ifdef COSP_GFDL
        call error_mesg ('dsd.F90',  & 
       'Error: Must specify a value for sigma_g', FATAL)
#else
