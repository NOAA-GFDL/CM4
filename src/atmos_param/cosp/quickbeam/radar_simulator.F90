...
#ifdef COSP_GFDL
               call error_mesg ('radar_simulator.F90', &
                'Re and Np set for the same volume & hydrometeor type.&
                          & Np is being ignored.', WARNING)
#else
...
#ifdef COSP_GFDL
!RSH:
!  In order to get reproducibility across physics windows, one must set the
!  parameter DO_LUT_TEST to .true., forcing execution of the if branch of
!  the following block. However doing so increases total model time by over
!  a factor of 2.  Therefore I have left the parameter set to .false..
!  As a result, the following diagnostics (and any others dependent on radar
!  reflectivity) become dependent on the physics_window / domain 
!  decomposition employed during a particular model run:
...
!    "clcalipso2_mdl"
!    "clcalipso2"
!  Differences are very slight and probably not of scientific consequence,
!  but do impact reproducibility.
!
!  If these diagnostics are required, they may be reproducibly obtained
!  (at great cost) by setting DO_LUT_TEST to .true., or by doing nothing
!  and recognizing that they will change slightly with a different domain
!  decomposition.
#endif
