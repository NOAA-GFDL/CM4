...
patch(reshape(u2x(kk),size(kk,1),size(kk,2))',...
      ...
      repmat(flintmax  ,size(kk,2),size(kk,1)),'w','edgecolor','k','clip','off','tag','m_grid_fancybox1');
patch(reshape(l2x(kk),size(kk,1),size(kk,2))',...
      reshape(l2y(kk),size(kk,1),size(kk,2))',...
      repmat(flintmax-1,size(kk,2),size(kk,1)),'k','clip','off','tag','m_grid_fancybox1');
...
line(reshape(mean(l2x(:,kk)),k1,k2),reshape(mean(l2y(:,kk))',k1,k2),...
     repmat(flintmax,k1,k2),'color','k','clip','off','tag','m_grid_fancybox2');
